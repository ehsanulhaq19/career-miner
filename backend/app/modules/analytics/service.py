...
from app.modules.analytics.schemas import AnalyticsHourlyRow, AnalyticsSummaryResponse
...
async def get_analytics_summary(
    ...
    and per-hour UTC buckets for dashboard visualizations.
    """
    ...
    sw_runs, sw_scraped, sw_by_hour = analytics_crud.aggregate_scrap_web_from_rows(
        scrap_web_rows
    )
    sc_runs, sc_scraped, sc_by_hour = analytics_crud.aggregate_scrap_client_from_rows(
        scrap_client_rows
    )
    ...
    email_by_status = await analytics_crud.count_email_logs_by_status_for_user_job_applications(
        db, user_id, d_from, d_to
    )
    ...
    jobs_by_hour = await analytics_crud.career_jobs_created_by_hour(
        db, d_from, d_to, user_id
    )
    clients_by_hour = await analytics_crud.career_clients_created_by_hour(
        db, d_from, d_to, user_id
    )
    apps_by_hour = await analytics_crud.job_applications_created_by_hour(
        db, user_id, d_from, d_to
    )
    emails_by_hour = await analytics_crud.email_logs_by_hour_and_status_for_user_job_applications(
        db, user_id, d_from, d_to
    )
    wf_by_hour = await analytics_crud.completed_workflow_executions_by_hour(
        db, user_id, d_from, d_to
    )

    hourly: list[AnalyticsHourlyRow] = []
    for hour_start in analytics_crud.iter_hours_inclusive(d_from, d_to):
        sw_h = sw_by_hour.get(hour_start, (0, 0))
        sc_h = sc_by_hour.get(hour_start, (0, 0))
        em_hour = emails_by_hour.get(hour_start, {})
        em_ok, em_er = analytics_crud.split_email_status_totals(em_hour)
        hourly.append(
            AnalyticsHourlyRow(
                hour_start=hour_start,
                scrap_web_jobs_run=sw_h[0],
                scrap_web_scraped_records=sw_h[1],
                scrap_client_jobs_run=sc_h[0],
                scrap_client_scraped_records=sc_h[1],
                jobs_created=jobs_by_hour.get(hour_start, 0),
                clients_created=clients_by_hour.get(hour_start, 0),
                job_applications_created=apps_by_hour.get(hour_start, 0),
                job_application_emails_success=em_ok,
                job_application_emails_error=em_er,
                workflows_completed=wf_by_hour.get(hour_start, 0),
            )
        )
